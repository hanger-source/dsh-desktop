// 排队消息 —— Client 半
// 使用 DSH 0.1.7 的 Inbox 投影与 pending submission echo，保留会话体验原有的
// 多行预览、逐条展开和大尺寸多行编辑器，不复制旧版 session.queue 状态。
return {
  ...
  apply(ctx) {
    styles.insert(`
      ...
      .dsh-flow-queue-panel{position:relative;isolation:isolate;overflow:hidden;width:100%;padding:2px 0;border-radius:12px 12px 0 0}
      .dsh-flow-queue-panel::before{position:absolute;z-index:-1;inset:0;border-radius:inherit;background:var(--dsw-specific-menu);backdrop-filter:var(--dsw-menu-backdrop-filter);content:"";pointer-events:none}
      .dsh-flow-queue-panel::after{position:absolute;inset:0;border:.5px solid var(--dsw-alias-border-l1);border-bottom:0;border-radius:inherit;content:"";pointer-events:none}
      .dsh-flow-queue-head{box-sizing:border-box;display:flex;align-items:center;gap:10px;width:100%;height:36px;padding:4px 12px;border:0;border-radius:8px;background:transparent;color:var(--dsw-alias-label-primary);font-size:13px;font-weight:500;line-height:24px;text-align:left;cursor:pointer}
      .dsh-flow-queue-head:focus-visible{outline:2px solid var(--dsw-alias-label-tertiary);outline-offset:-2px}
      .dsh-flow-queue-count{flex:1 1 auto;min-width:0}
      .dsh-flow-queue-status{flex:none;color:var(--dsw-alias-label-tertiary);font:var(--dsw-font-xs-13);white-space:nowrap}
      ...
      .dsh-flow-queue-list[hidden]{display:none!important}
      ...
      .dsh-flow-queue-lead{display:grid;flex:none;place-items:center;width:14px;height:18px;color:var(--dsw-alias-label-tertiary);font-size:14px;line-height:18px}
      ...
      .dsh-flow-queue-attachments{display:flex;flex:none;align-items:center;gap:4px;min-width:0;max-width:55%;overflow:hidden}
      .dsh-flow-queue-thumb{display:block;flex:none;width:32px;height:32px;object-fit:cover;border:.5px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-base)}
      .dsh-flow-queue-file{box-sizing:border-box;display:inline-flex;flex:0 180px;align-items:center;gap:4px;min-width:74px;height:28px;padding:0 7px;overflow:hidden;border:.5px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary-dimmed);font:var(--dsw-font-xs-13)}
      .dsh-flow-queue-file-name{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      ...
      .dsh-flow-queue-action{appearance:none;display:grid;flex:none;place-items:center;width:28px;height:28px;padding:0;border:0;border-radius:999px;background:transparent;color:var(--dsw-alias-label-tertiary);font-size:14px;line-height:1;cursor:pointer}
      ...
      .dsh-flow-queue-editor{box-sizing:border-box;width:100%;min-height:92px;max-height:180px;resize:vertical;padding:9px 10px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;outline:none;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font:var(--dsw-font-xs-13);line-height:18px}
    ...
    const icons = {
      ...
      send: { viewBox: '0 0 14 14', paths: ['M7.24707 1.01771C7.52897 1.07653 7.77619 1.19694 8.00391 1.38001C8.19202 1.53136 8.39884 1.73784 8.61914 1.95814L12.6396 5.9806L11.6299 6.99134L7.71484 3.0763V13.0001H6.28516V3.0763L2.36914 6.99134L1.35938 5.9806L5.38086 1.95814C5.60116 1.73784 5.80798 1.53136 5.99609 1.38001C6.19476 1.22027 6.4385 1.06739 6.75195 1.01771C6.91296 0.992304 7.07471 0.997504 7.24707 1.01771Z'] },
      save: { viewBox: '0 0 16 16', paths: ['M15.0498 3.92579L8.49512 12.3818C8.25774 12.6881 8.04517 12.9645 7.84668 13.1689C7.63957 13.3823 7.38732 13.5841 7.04492 13.6719C6.86373 13.7183 6.6757 13.7346 6.48926 13.7197C6.13666 13.6915 5.8528 13.5355 5.6123 13.3604C5.38201 13.1926 5.12573 12.9567 4.83984 12.6953L1.03125 9.21289L1.96875 8.1875L5.77734 11.6699C6.08684 11.9529 6.27773 12.1249 6.43066 12.2363C6.50183 12.2882 6.54699 12.3135 6.57324 12.3252C6.58525 12.3305 6.59269 12.3322 6.5957 12.333C6.59802 12.3336 6.59961 12.334 6.59961 12.334C6.63317 12.3367 6.66758 12.3335 6.7002 12.3252C6.7002 12.3252 6.70211 12.3251 6.7041 12.3242C6.70698 12.3229 6.71348 12.319 6.72461 12.3115C6.74849 12.2956 6.78843 12.2642 6.84961 12.2012C6.98138 12.0654 7.13957 11.8628 7.39648 11.5313L13.9502 3.07422L15.0498 3.92579Z'] },
      cancel: { viewBox: '0 0 16 16', paths: ['M14.1168 13.197L13.197 14.1167L1.8833 2.80303L2.80309 1.88324L14.1168 13.197Z', 'M13.197 1.88326L14.1168 2.80305L2.80309 14.1168L1.8833 13.197L13.197 1.88326Z'] },
    }
    ...
    function textOf(content) {
      if (!Array.isArray(content) || !content.every(block => block && block.type === 'text')) return null
    ...
    function attachmentsOf(content) {
      return Array.isArray(content) ? content.filter(block => block && (block.type === 'image' || block.type === 'file')) : []
    }
    ...
    function QueueImage({ attachment, loadImage }) {
    ...
    function QueueAttachments({ attachments, loadImage }) {
      if (attachments.length === 0) return null
      return React.createElement('span', { className: 'dsh-flow-queue-attachments' }, attachments.map((block, index) => {
        if (block.type === 'image') {
          return React.createElement(QueueImage, {
            key: String(block.attachment?.attachmentId || index),
        ...
        return React.createElement('span', {
          key: String(block.attachment?.attachmentId || index),
          className: 'dsh-flow-queue-file',
          title: block.attachment?.name || '文件',
        }, React.createElement('span', { className: 'dsh-flow-queue-file-name' }, block.attachment?.name || '文件'))
      }))
    }

    function Action({ name, label, disabled, onClick }) {
      return React.createElement('button', {
        type: 'button',
        className: 'dsh-flow-queue-action',
        'aria-label': label,
        title: label,
        disabled,
        onClick,
    ...
    function QueueDock({ useSession, useProjection, updateQueue, notify, loadImage }) {
      ...
      const queue = React.useMemo(() => {
        ...
        const inTranscript = new Set(pendingSubmissions
          .filter(item => item.placement === 'transcript')
          .map(item => item.requestId))
        return inTranscript.size === 0 ? rows : rows.filter(row => {
          const source = row.source
          return source?.kind !== 'user' || !('rpcId' in source) || !inTranscript.has(source.rpcId)
        })
      }, [inbox, pendingSubmissions])
      const pendingQueue = React.useMemo(() => {
        const admitted = new Set(queue.flatMap(row => {
          const source = row.source
          return source?.kind === 'user' && 'rpcId' in source ? [source.rpcId] : []
        }))
      ...
      const mutable = subagent === null || subagent?.address?.mode === 'continuable'
      ...
      }, [collapsed, editing, mutable, queue, rowCount])

      if (rowCount === 0) return null
      const interactionActive = mutable && (editing !== null || busy !== null)
      const listVisible = rowCount === 1 || !collapsed || interactionActive
      ...
      const act = async (id, action, failure) => {
        ...
        try {
          ...
          const detail = String(actionError?.message || actionError)
      ...
      const save = async () => {
        ...
        const saved = await act(editing.id, {
          kind: 'edit',
      ...
      const rows = queue.map(row => {
        ...
        const editableText = textOf(row.content)
        const attachments = attachmentsOf(row.content)
        ...
        if (isEditing) {
          return React.createElement('li', { key: row.id, className: 'dsh-flow-queue-item' },
            React.createElement('textarea', {
              autoFocus: true,
              className: 'dsh-flow-queue-editor',
              value: editing.text,
              'aria-label': '编辑排队消息',
            ...
            React.createElement('div', { className: 'dsh-flow-queue-edit-foot' },
              ...
              React.createElement(Action, { name: 'cancel', label: '取消编辑', disabled: busy !== null, onClick: () => setEditing(null) }),
              React.createElement(Action, { name: 'save', label: '保存排队消息', disabled: busy !== null || editing.text.trim() === '', onClick: () => void save() })))
        }
        ...
        return React.createElement('li', { key: row.id, className: 'dsh-flow-queue-item' },
          React.createElement('div', { className: 'dsh-flow-queue-line' },
            ...
            React.createElement(QueueAttachments, { attachments, loadImage }),
            ...
            mutable ? React.createElement('div', { className: 'dsh-flow-queue-actions' },
              React.createElement(Action, { name: 'edit', label: '编辑排队消息', disabled: busy !== null || editableText === null, onClick: () => editableText !== null && setEditing({ id: row.id, text: editableText }) }),
              React.createElement(Action, { name: 'trash', label: '移除排队消息', disabled: busy !== null, onClick: () => void act(row.id, { kind: 'remove' }, '移除排队消息失败') }),
              React.createElement(Action, { name: 'send', label: '立即发送排队消息', disabled: busy !== null || !running, onClick: () => void act(row.id, { kind: 'steer' }, '立即发送排队消息失败') })) : null),
          isLong(text) ? React.createElement('button', {
            type: 'button',
            className: 'dsh-flow-queue-expand',
            onClick: () => setExpanded(values => ({ ...values, [row.id]: !full })),
          }, full ? '收起' : '展开全部') : null)
      })

      rows.push(...pendingQueue.map(submission => {
        const attachments = Array.isArray(submission.attachments) ? submission.attachments : []
        return React.createElement('li', {
          key: submission.requestId,
          className: 'dsh-flow-queue-item',
          'data-submission-echo': '',
        }, React.createElement('div', { className: 'dsh-flow-queue-line' },
          React.createElement('span', { className: 'dsh-flow-queue-lead', 'aria-hidden': 'true' }, React.createElement(Icon, { name: 'queue' })),
          attachments.length > 0 ? React.createElement('span', { className: 'dsh-flow-queue-attachments' }, attachments.map((attachment, index) => {
            if (attachment.type === 'image') return React.createElement('img', {
              key: attachment.value.previewUrl + ':' + index,
              className: 'dsh-flow-queue-thumb',
              src: attachment.value.previewUrl,
              alt: '排队消息图片',
            })
            return React.createElement('span', {
              key: String(attachment.value.attachmentId || index),
              className: 'dsh-flow-queue-file',
            }, React.createElement('span', { className: 'dsh-flow-queue-file-name' }, attachment.value.name || '文件'))
          })) : null,
          React.createElement('span', { className: 'dsh-flow-queue-preview' }, submission.text),
          React.createElement('span', { className: 'dsh-flow-queue-status', role: 'status' }, '发送中…')))
      }))
      ...
      return React.createElement('div', { className: 'dsh-flow-queue', 'data-dsh-flow-queue': '' },
        React.createElement('div', { className: 'dsh-flow-queue-panel' },
          rowCount > 1 ? React.createElement('button', {
            type: 'button',
            className: 'dsh-flow-queue-head',
          ...
          !listVisible && pendingQueue.length > 0 ? React.createElement('span', { className: 'dsh-flow-queue-status', role: 'status' }, '发送中…') : null,
          React.createElement('span', { className: 'dsh-flow-queue-lead', 'aria-hidden': 'true' }, listVisible ? '⌄' : '⌃')) : null,
          React.createElement('ol', { className: 'dsh-flow-queue-list', hidden: !listVisible }, rows),
