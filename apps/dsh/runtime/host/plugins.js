...
const GENERATED_CLIENT_HEADER = '// Generated from client/*.js. Do not edit this file directly.\n'
...
class PluginRepository {
  ...
  clientSource(directory) {
    const flatPath = Path.join(directory, 'code.client.js')
    const partsPath = Path.join(directory, 'client')
    const hasFlat = Fs.existsSync(flatPath)
    const partNames = Fs.existsSync(partsPath)
      ? Fs.readdirSync(partsPath).filter(name => name.endsWith('.js')).sort()
      : []
    if (partNames.length === 0) return hasFlat ? Fs.readFileSync(flatPath, 'utf8') : ''

    const definitions = partNames.map((name, index) => [
      'const __dshClientPart' + index + ' = (() => {',
      Fs.readFileSync(Path.join(partsPath, name), 'utf8'),
      '})()',
    ].join('\n'))
    const variables = partNames.map((_name, index) => '__dshClientPart' + index)
    const composed = [
      ...definitions,
      'const __dshClientParts = [' + variables.join(', ') + ']',
    ...
    const generated = GENERATED_CLIENT_HEADER + composed + '\n'
    if (hasFlat && Fs.readFileSync(flatPath, 'utf8') !== generated) {
      throw new Error('code.client.js 已过期，请从 client/*.js 重新生成')
    }
    return generated
  }
  ...
  sources() {
    ...
    for (const key of Fs.readdirSync(root).sort()) {
      ...
      let client
      try {
        host = Fs.existsSync(hostPath) ? Fs.readFileSync(hostPath, 'utf8') : ''
        client = this.clientSource(directory)
      } catch (error) {
        this.log('[plugins] ' + key + ' code 无效：' + error.message)
        continue
      }
